...
class UserForm(forms.Form):
    ...
    #form elements
    firstname=forms.CharField(max_length=100,required=True,validators=[alphanumeric],widget=forms.TextInput(attrs={'placeholder':'Firstname','class':'form-control'}))  
...
class BookingForm(forms.Form):
    numbers=RegexValidator(r'^[0-9]*$','only numbers allowed')
    mobile=forms.CharField(max_length=10,min_length=10,validators=[numeric])
    date=forms.DateTimeField(widget=forms.DateTimeInput(attrs={'placeholder':'%m/%d/%Y %H:%M'}),input_formats="%m/%d/%Y %H:%M")
